 ...
 *
 *)
unit TSS_FE_KioskInternetForm_unit;
{$I cef.inc}
interface
uses
  {$IFDEF DELPHI16_UP}
  ...
  uCEFSentinel, uCEFChromiumCore, Vcl.Touch.GestureMgr, Vcl.Buttons,Vcl.Touch.KeyboardTypes;
const
...
type
  TKioskInternetForm = class(TForm)
    chrmosr: TChromium;
...
var
  KioskInternetForm: TKioskInternetForm;
...
begin
  ...
  GlobalCEFApp.Locale                     := 'de_DE';
  //GlobalCEFApp.EnableGPU                  := True;
end;
procedure TKioskInternetForm.AppEventsMessage(var Msg: tagMSG; var Handled: Boolean);
var
...
procedure TKioskInternetForm.chrmosrBeforeClose(Sender: TObject; const browser: ICefBrowser);
begin
  FCanClose := True;
  PostMessage(Handle, WM_CLOSE, 0, 0);
end;
procedure TKioskInternetForm.chrmosrBeforeContextMenu(      Sender  : TObject;
...
procedure TKioskInternetForm.chrmosrContextMenuCommand(      Sender     : TObject;
                                           const browser    : ICefBrowser;
                                           const frame      : ICefFrame;
                                           const params     : ICefContextMenuParams;
                                                 commandId  : Integer;
...
procedure TKioskInternetForm.chrmosrBeforePopup(      Sender             : TObject;
                                    const browser            : ICefBrowser;
                                    const frame              : ICefFrame;
...
procedure TKioskInternetForm.chrmosrCursorChange(      Sender           : TObject;
                                     const browser          : ICefBrowser;
...
procedure TKioskInternetForm.chrmosrGetScreenInfo(      Sender     : TObject;
                                      const browser    : ICefBrowser;
...
procedure TKioskInternetForm.chrmosrGetScreenPoint(      Sender  : TObject;
                                       const browser : ICefBrowser;
                                             viewX   : Integer;
                                             viewY   : Integer;
                                       var   screenX : Integer;
...
procedure TKioskInternetForm.chrmosrGetViewRect(      Sender  : TObject;
                                    const browser : ICefBrowser;
                                    var   rect    : TCefRect);
var
  TempScale : single;
...
procedure TKioskInternetForm.chrmosrPaint(      Sender          : TObject;
                              const browser         : ICefBrowser;
...
procedure TKioskInternetForm.chrmosrPopupShow(      Sender  : TObject;
                                  const browser : ICefBrowser;
                                        show    : Boolean);
begin
  if show then
...
procedure TKioskInternetForm.chrmosrPopupSize(      Sender  : TObject;
                                  const browser : ICefBrowser;
                                  const rect    : PCefRect);
begin
...
procedure TKioskInternetForm.chrmosrTooltip(Sender: TObject; const browser: ICefBrowser; var text: ustring; out Result: Boolean);
begin
  Panel1.hint     := text;
...
function TKioskInternetForm.LoadLayout(const LayoutName: string): TVirtualKeyLayout;
var
...
procedure TKioskInternetForm.chrmosrVirtualKeyboardRequested(      Sender     : TObject;
                                                 const browser    : ICefBrowser;
...
function TKioskInternetForm.getModifiers(Shift: TShiftState): TCefEventFlags;
begin
...
function TKioskInternetForm.GetButton(Button: TMouseButton): TCefMouseButtonType;
begin
  case Button of
...
procedure TKioskInternetForm.WMMove(var aMessage : TWMMove);
begin
  inherited;
  if (chrmosr <> nil) then chrmosr.NotifyMoveOrResizeStarted;
end;
procedure TKioskInternetForm.WMMoving(var aMessage : TMessage);
begin
  inherited;
  if (chrmosr <> nil) then chrmosr.NotifyMoveOrResizeStarted;
end;
procedure TKioskInternetForm.WMCaptureChanged(var aMessage : TMessage);
begin
  inherited;
  if (chrmosr <> nil) then chrmosr.SendCaptureLostEvent;
end;
procedure TKioskInternetForm.WMCancelMode(var aMessage : TMessage);
begin
  inherited;
  if (chrmosr <> nil) then chrmosr.SendCaptureLostEvent;
end;
procedure TKioskInternetForm.WMEnterMenuLoop(var aMessage: TMessage);
begin
  inherited;
  if (aMessage.wParam = 0) and (GlobalCEFApp <> nil) then GlobalCEFApp.OsmodalLoop := True;
end;
procedure TKioskInternetForm.WMExitMenuLoop(var aMessage: TMessage);
begin
  inherited;
  if (aMessage.wParam = 0) and (GlobalCEFApp <> nil) then GlobalCEFApp.OsmodalLoop := False;
end;
procedure TKioskInternetForm.FormAfterMonitorDpiChanged(Sender: TObject; OldDPI, NewDPI: Integer);
begin
...
procedure TKioskInternetForm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  chrmosr.CloseBrowser(True);
end;

procedure TKioskInternetForm.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CanClose := FCanClose;
...
procedure TKioskInternetForm.FormCreate(Sender: TObject);
var
...
procedure TKioskInternetForm.FormDestroy(Sender: TObject);
begin
  chrmosr.ShutdownDragAndDrop;
  if (FPopUpBitmap <> nil) then FreeAndNil(FPopUpBitmap);
  if (FResizeCS    <> nil) then FreeAndNil(FResizeCS);
end;
procedure TKioskInternetForm.FormHide(Sender: TObject);
begin
  chrmosr.SetFocus(False);
  chrmosr.WasHidden(True);
end;
procedure TKioskInternetForm.FormShow(Sender: TObject);
begin
  if chrmosr.Initialized then
    ...
    begin
      ...
      chrmosr.options.AcceptLanguageList:='de_DE';
...
procedure TKioskInternetForm.Panel1Click(Sender: TObject);
begin
  Panel1.SetFocus;
end;
procedure TKioskInternetForm.Panel1CustomTouch(Sender: TObject; var aMessage: TMessage; var aHandled: Boolean);
var
...
procedure TKioskInternetForm.Panel1MouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
...
procedure TKioskInternetForm.Panel1MouseLeave(Sender: TObject);
var
  TempEvent : TCefMouseEvent;
  TempPoint : TPoint;
  TempTime  : integer;
...
procedure TKioskInternetForm.Panel1MouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  TempEvent : TCefMouseEvent;
  TempTime  : integer;
begin
...
procedure TKioskInternetForm.Panel1MouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  TempEvent : TCefMouseEvent;
begin
  {$IFDEF DELPHI14_UP}
...
procedure TKioskInternetForm.Panel1PointerDown(Sender: TObject; var aMessage: TMessage; var aHandled: Boolean);
begin
...
procedure TKioskInternetForm.Panel1PointerUp(Sender: TObject; var aMessage: TMessage; var aHandled: Boolean);
begin
  aHandled := Panel1.Focused and
              (GlobalCEFApp <> nil) and
              ArePointerEventsSupported and
...
procedure TKioskInternetForm.Panel1PointerUpdate(Sender: TObject; var aMessage: TMessage; var aHandled: Boolean);
begin
  aHandled := Panel1.Focused and
              (GlobalCEFApp <> nil) and
              ArePointerEventsSupported and
...
procedure TKioskInternetForm.Panel1Resize(Sender: TObject);
begin
  DoResize;
end;
procedure TKioskInternetForm.PendingResizeMsg(var aMessage : TMessage);
begin
  DoResize;
end;
procedure TKioskInternetForm.ShowKeyboardMsg(var aMessage : TMessage);
begin
  TouchKeyboard1.Visible := True;
end;

procedure TKioskInternetForm.HideKeyboardMsg(var aMessage : TMessage);
begin
  TouchKeyboard1.Visible := False;
end;
procedure TKioskInternetForm.DoResize;
begin
  try
    FResizeCS.Acquire;
    if FResizing then
...
procedure TKioskInternetForm.InitializeLastClick;
begin
  FLastClickCount   := 1;
  FLastClickTime    := 0;
  FLastClickPoint.x := 0;
...
function TKioskInternetForm.CancelPreviousClick(x, y : integer; var aCurrentTime : integer) : boolean;
begin
...
function TKioskInternetForm.ArePointerEventsSupported : boolean;
begin
...
function TKioskInternetForm.HandlePointerEvent(var aMessage : TMessage) : boolean;
const
  PT_TOUCH = 2;
  PT_PEN   = 3;
var
...
function TKioskInternetForm.HandlePenEvent(const aID : uint32; aMsg : cardinal) : boolean;
var
...
function TKioskInternetForm.HandleTouchEvent(const aID : uint32; aMsg : cardinal) : boolean;
var
...
procedure TKioskInternetForm.Panel1Enter(Sender: TObject);
begin
  chrmosr.SetFocus(True);
end;

procedure TKioskInternetForm.Panel1Exit(Sender: TObject);
begin
  TouchKeyboard1.Visible := False;
  chrmosr.SetFocus(False);
end;

procedure TKioskInternetForm.Timer1Timer(Sender: TObject);
begin
...
procedure TKioskInternetForm.SpeedButton1Click(Sender: TObject);
begin
  KioskInternetform.close;
end;

procedure TKioskInternetForm.SpeedButton2Click(Sender: TObject);
begin
  chrmosr.GoBack
end;

procedure TKioskInternetForm.SpeedButton3Click(Sender: TObject);
begin
  chrmosr.GoForward;
end;
...
procedure TKioskInternetForm.SpeedButton4Click(Sender: TObject);
var c:char;
begin
//  PostKeyEx32(ord('q'),[ssctrl,ssAlt],false);
